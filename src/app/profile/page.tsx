...
export default function ProfilePage() {
    ...
    useEffect(() => {
        fetch("/api/user/points")
            .then(res => res.json())
            .then(data => {
                if (!data.error) setStats({ points: data.points || 0 });
            })
            .catch(console.error)
    ...
    const menuItems = [
        {
            title: "Poin Belanja",
            icon: <Coins className="w-5 h-5 text-[var(--accent-primary)]" />,
            href: "/profile/redeem",
            description: "Lihat riwayat perolehan poin belanja Anda"
        },
        {
            title: "Keamanan Akun",
            icon: <Lock className="w-5 h-5 text-[var(--accent-primary)]" />,
            href: "/profile/security",
            description: "Kelola keamanan akun & hapus akun secara permanen"
        }
    ];
    ...
    return (
        <div style={{ minHeight: "calc(100vh - var(--header-height, 56px))", background: "var(--bg-base)", padding: "24px 16px 40px", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ width: "100%", maxWidth: "520px" }}>
                <div className="card space-y-5" style={{ padding: "24px 24px 28px", position: "relative" }}>
                ...
                <div className="card-lg" style={{ padding: "24px" }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                        <div style={{ width: "72px", height: "72px", background: "var(--accent-muted)", border: "1px solid var(--accent-border)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "16px" }}>
                            <UserIcon style={{ width: "32px", height: "32px", color: "var(--accent-primary)" }} />
                        </div>
                ...
                <div className="card" style={{ padding: "16px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                            <div style={{ width: "38px", height: "38px", background: "var(--gold-muted)", borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        ...
                        {menuItems.map((item, idx) => (
                            ...
                            >
                                <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                                    <div style={{ width: "38px", height: "38px", background: "var(--bg-elevated)", border: "var(--border-default)", borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "border-color 150ms ease, background 150ms ease" }} className="group-hover:border-[var(--accent-border)] group-hover:bg-[var(--accent-subtle)]">
                                        {item.icon}
                                    </div>
                                    <div>
                                ...
                                <ChevronRight style={{ width: "15px", height: "15px", color: "var(--text-muted)", transition: "color 150ms ease" }} className="group-hover:text-[var(--accent-primary)]" />
                            </Link>
                        ))}
                    </div>
                </div>
