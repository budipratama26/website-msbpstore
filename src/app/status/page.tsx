...
export default function StatusPage() {
    ...
    const checkStatus = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!contentType.includes("application/json")) { setError("Pesanan tidak ditemukan. Pastikan Order ID sudah benar."); return; }
            const data = await res.json();
            if (!res.ok) setError(data.error || "Pesanan tidak ditemukan.");
            else setOrder(data);
        } catch { setError("Pesanan tidak ditemukan. Pastikan Order ID sudah benar."); }
        finally { setLoading(false); }
    };

    const getStatusStyle = (status: string) => {
        switch (status.toUpperCase()) {
            case "SUCCESS": case "COMPLETED": case "PAID": return "badge-success";
    ...
    return (
        <div style={{ minHeight: "calc(100vh - var(--header-height, 60px))", background: "var(--bg-base)", padding: "24px 16px 60px" }}>
            <div style={{ maxWidth: "800px", margin: "0 auto" }}>
                <div className="card animate-fade-in" style={{ padding: "24px 24px 32px", minHeight: "400px" }}>
                    ...
                    <div style={{ marginBottom: "24px" }}>
                        <form onSubmit={checkStatus} style={{ position: "relative" }}>
                            ...
                            <input type="text" placeholder="Masukkan nomor pesanan Anda" value={orderId} onChange={(e) => setOrderId(e.target.value)} className="form-input form-input-icon" style={{ paddingRight: "96px" }} />
                            <button type="submit" disabled={loading || !orderId.trim()} className="btn btn-primary btn-sm" style={{ position: "absolute", right: "8px", top: "50%", transform: "translateY(-50%)" }}>
                    ...
                    <div style={{ minHeight: "200px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                        {error && (
                            <div style={{ background: "var(--error-muted)", border: "1px solid rgba(248,113,113,0.3)", borderRadius: "var(--radius-xl)", padding: "32px 20px", textAlign: "center" }}>
                        ...
                        {!order && !error && !loading && (
                            <div className="empty-state" style={{ border: "none", background: "transparent" }}>
                                <div className="empty-icon"><Receipt className="w-6 h-6" /></div>
                        ...
                        {order && (
                            <div style={{ overflow: "hidden", border: "1px solid var(--bg-border)", borderRadius: "var(--radius-xl)", background: "var(--bg-base)" }} className="animate-fade-in">
                                ...
                                {order.sn && (
                                    <div style={{ borderTop: "var(--border-default)", background: "var(--success-muted)", padding: "16px 20px" }}>
                                        <div style={{ display: "flex", alignItems: "flex-start", gap: "12px" }}>
                                            <div style={{ width: "34px", height: "34px", background: "rgba(52,211,153,0.15)", border: "1px solid rgba(52,211,153,0.4)", borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: "2px" }}>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ background: "var(--bg-elevated)", padding: "12px", borderRadius: "var(--radius-md)", border: "1px solid rgba(52,211,153,0.3)", fontFamily: "monospace", fontSize: "14px", fontWeight: 700, color: "var(--success)", userSelect: "all", cursor: "copy", wordBreak: "break-all" }}>
                                                    {order.sn}
                                                </div>
                                            </div>
                                        </div>
