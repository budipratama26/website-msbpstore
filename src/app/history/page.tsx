...
export default function OrderHistoryPage() {
    ...
    if (sessionStatus === "unauthenticated") {
        return (
            <div style={{ minHeight: "100vh", background: "var(--bg-base)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 24px 24px", textAlign: "center" }}>
                ...
                <p style={{ color: "var(--text-secondary)", maxWidth: "400px", marginBottom: "32px", fontSize: "14px", lineHeight: 1.65 }}>Halaman Riwayat Pesanan hanya dapat diakses oleh member yang sudah login. Silakan masuk untuk melihat transaksi Anda.</p>
    ...
    return (
        <div style={{ minHeight: "calc(100vh - var(--header-height, 56px))", background: "var(--bg-base)", padding: "24px 16px 40px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                <div className="card" style={{ padding: "20px", marginBottom: "0" }}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div className="page-header mb-0">
                            ...
                            <div>
                                <h1 className="text-xl font-bold tracking-tight text-[var(--text-primary)]">Riwayat Pesanan</h1>
                                <p className="text-sm text-[var(--text-muted)] font-medium mt-0.5">Pantau semua transaksi top-up game Anda.</p>
                            </div>
                        </div>
                        ...
                        <div style={{ position: "relative", width: "100%", maxWidth: "300px" }}>
                            ...
                            <input
                                type="text"
                                placeholder="Cari ID Pesanan atau Game..."
                                value={searchQuery}
                    ...
                    <div className="space-y-3">
                        {filteredOrders.length > 0 ? (
                            filteredOrders.map((order) => {
                                ...
                                const badgeClass = ["SUCCESS","COMPLETED","PAID"].includes(currentStatus.toUpperCase()) ? "badge-success" :
                                    currentStatus.toUpperCase() === "PENDING" ? "badge-warning" : "badge-danger";

                                return (
                                    <div key={order.id} style={{ background: "var(--bg-elevated)", border: "var(--border-default)", borderRadius: "var(--radius-lg)", padding: "16px" }}>
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    ...
                                                    <div style={{ marginTop: "6px" }}>
                                                        ...
                                                        >
                                                            ...
                                                            {copiedId === order.id
                                                                ? <Check style={{ width: "10px", height: "10px", color: "var(--success)" }} />
            ...
            {/* QR Code Modal */}
            {selectedQrOrder && (
                <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg-overlay)", backdropFilter: "blur(4px)", padding: "16px" }}>
                    <div style={{ background: "var(--bg-surface)", border: "var(--border-default)", borderRadius: "var(--radius-xl)", width: "100%", maxWidth: "380px", overflow: "hidden", boxShadow: "var(--shadow-lg)", position: "relative" }} className="animate-fade-in">
                        {/* Modal Header */}
                        ...
                        {paymentSuccess ? (
                            <div style={{ padding: "32px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                                <div style={{ width: "72px", height: "72px", background: "var(--success-muted)", border: "1px solid rgba(16,185,129,0.4)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "20px" }}>
                            ...
                            <div style={{ padding: "20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
                                ...
                                <div style={{ background: "var(--bg-elevated)", padding: "12px", borderRadius: "var(--radius-lg)", border: `2px dashed ${timeLeft === 0 ? "var(--bg-border)" : "var(--accent-border)"}`, marginBottom: "20px", opacity: timeLeft === 0 ? 0.4 : 1, position: "relative" }}>
                                    <div style={{ width: "176px", height: "176px", position: "relative" }}>
                                        <img
                                            src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(selectedQrOrder.paymentData || "")}`}
                                            alt="QRIS Code"
                                            style={{ width: "100%", height: "100%", objectFit: "contain", filter: timeLeft === 0 ? "grayscale(1)" : "none" }}
                                        />
                                        {timeLeft === 0 && (
